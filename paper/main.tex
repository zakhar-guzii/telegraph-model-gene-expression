...
Because every result in this paper is an average over pseudo-random draws, we state the generator
explicitly. All experiments use the Mersenne Twister MT19937~\cite{matsumoto1998} through
\texttt{numpy.random}~\cite{harris2020}. Its period of $2^{19937}-1$ is vast compared with the
$\mathcal{O}(10^7)$ variates consumed by the largest run reported here, so exhausting the generator
is not a concern. A seed is fixed for each experiment, so every figure in this paper is exactly
...
\end{equation}
By the Central Limit Theorem, the sampler's error decays as
...
(Figure~\ref{fig:wf-expr}) is set by the burst size $\ksyn/\koff$ of~\eqref{eq:burst} at fixed $\kon$: a strong
...
covariance equation~\eqref{eq:dCRG} predicts and in agreement with the exact analysis of the
switching limits in~\cite{iyerbiswas2009}. Second, the mean is governed by
$\muR^{ss}=(\ksyn/\kdeg)\,\kon/(\kon+\koff)$, so with $\kon$ fixed the burst size~\eqref{eq:burst} sets
